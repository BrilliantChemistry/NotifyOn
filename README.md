...
| `email` | `hash` | (Default: `nil`) If present, it will attempt to send an email notification. Options: <ul><li>`default_from` (`boolean`) Use the default email address as sender instead of the notification's sender.</li><li>`template` (`string` or `symbol`) The name of the email template to render.</li><li>`unless` (`symbol`) a method on the trigger that returns a boolean. If `true` the email **will not** be sent. It falls back to methods on the notification if it doesn't find a match.</li> |
